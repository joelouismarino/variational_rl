...
def print_step_metrics(step_num, n_episodes, model, observation, reward, done):
    ...
    print('             Done CLL: ' + str(model.done_variable.cond_log_likelihood(done).sum().item()))
...
def print_episode_metrics(episode, episode_log):
    ...
    print('         Done CLL: ' + str(np.mean(episode_log['done_cll'])))
