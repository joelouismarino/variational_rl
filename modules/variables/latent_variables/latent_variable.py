...
class LatentVariable(nn.Module):
    ...
    def init_approx_post(self):
        ...
        if self.approx_post is not None:
            ...
            self.approx_post.reset(dist_params=parameters)
    ...
    def log_importance_weights(self):
        ...
        sample = sample.view(self.approx_post._n_samples, -1, self.n_variables)
        # expand the distributions to handle evaluating multiple samples
        expanded_prior = self.prior.dist.expand(torch.Size([self.approx_post._n_samples]) + self.prior.dist.batch_shape)
        expanded_approx_post = self.approx_post.dist.expand(torch.Size([self.approx_post._n_samples]) + self.approx_post.dist.batch_shape)
    ...
    # elif prior_dist == 'TransformedTanh':
