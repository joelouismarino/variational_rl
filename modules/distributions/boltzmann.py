...
class Boltzmann(Distribution):
    ...
    Args:
        ...
        eta (torch.tensor):             the 'temperature' parameter [1]
    """
    arg_constraints = {'prior_log_probs': constraints.real,
                       'q_values': constraints.real,
                       'eta': constraints.positive}
    support = constraints.real
    has_rsample = False
    def __init__(self, prior_log_probs, q_values, eta, validate_args=None):
        ...
        values = torch.logsumexp(self.q_values / self.eta, dim=0, keepdim=True)
        values = values - torch.tensor(n_action_samples, dtype=torch.float32, device=eta.device).log()
        self.values = self.eta * values
    ...
    def log_prob(self, value):
        ...
        log_probs = self.prior_log_probs + self.advantages / self.eta
        return log_probs.repeat(1, 1, action_dim) / action_dim

    def get_weights(self):
        return torch.exp(self.advantages / self.eta)
