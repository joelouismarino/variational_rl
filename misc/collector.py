...
class Collector:
    ...
    def _collect_kl_objectives(self, on_policy_action, target_on_policy_action, valid, done):
        ...
        if self.agent.inf_target_kl:
            ...
            kl = kl_divergence(self.agent.approx_post, self.agent.target_approx_post, n_samples=self.agent.n_action_samples, sample=on_policy_action).sum(dim=1, keepdim=True)
    ...
    def _train_value_estimators(self):
        ...
        if 'q_value_models' in dir(self.agent.q_value_estimator):
            ...
            if self.agent.variance_weighted_value_loss:
                ...
                if True:
                    # from SUNRISE
                    q_target_weights += torch.sigmoid(-q_target_variance.sqrt() * 10)
                else:
